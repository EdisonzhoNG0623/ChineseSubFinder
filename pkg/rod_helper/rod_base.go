...
func NewBrowserEx(rodOptions *BrowserOptions) (*rod.Browser, error) {
	...
	if rodOptions.Settings.ExperimentalFunction.RemoteChromeSettings.Enable == false {
		...
		localChromeFPath = resolveLocalChromePath(localChromeFPath)
...
func resolveLocalChromePath(configuredPath string) string {
	if configuredPath != "" {
		return configuredPath
	}
	return os.Getenv("CSF_CHROME_BIN")
}
...
func NewBrowserBase(log *logrus.Logger, localChromeFPath, httpProxyURL string, loadAdblock bool, preLoadUrl ...string) (*rod.Browser, error) {
	// Rod 的 managed launcher 会共用浏览器下载缓存。首次启动时并发创建
	// 多个本地浏览器可能同时下载/解压同一 Chromium，导致 MustLaunch panic。
	// 这里只串行化创建和预加载阶段；函数返回后的浏览器操作不受影响。
	localBrowserLaunchMu.Lock()
	defer localBrowserLaunchMu.Unlock()
...
var (
	once                 sync.Once
	localBrowserLaunchMu sync.Mutex
)
