...
func (s *Supplier) CheckAlive() (bool, int64) {
	...
	result, page, err := rod_helper.HttpGetFromBrowser(browser, settings.Get().AdvancedSettings.SuppliersSettings.Zimuku.RootUrl, 15*time.Second)
	if err != nil {
		s.isAlive = false
		return false, 0
	}
	_ = page.Close()
	if strings.Contains(result, "网站防火墙") || strings.Contains(result, "请输入验证码后继续访问") {
		s.log.Warningln(s.GetSupplierName(), "CheckAlive blocked by site verification, disable for this run")
		s.isAlive = false
		return false, 0
	}
...
func (s *Supplier) OverDailyDownloadLimit() bool {
	supplierSettings := settings.Get().AdvancedSettings.SuppliersSettings.Zimuku
