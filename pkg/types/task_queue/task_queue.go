...
type OneJob struct {
	...
	SearchAliases                []string         `json:"search_aliases,omitempty"`                  // 实际参与供应商查询的规范化别名
	SearchEvidenceVersion        int              `json:"search_evidence_version,omitempty"`         // 搜索证据结构版本
	SearchFingerprint            string           `json:"search_fingerprint,omitempty"`              // 不含路径的搜索证据指纹
	LastAttemptSearchFingerprint string           `json:"last_attempt_search_fingerprint,omitempty"` // 上次实际搜索使用的证据指纹
	LastAttemptPolicyFingerprint string           `json:"last_attempt_policy_fingerprint,omitempty"` // 上次实际搜索使用的源/查询策略指纹
	LegacySearchEvidenceBaseline bool             `json:"legacy_search_evidence_baseline,omitempty"` // 旧队列首次元数据合并保持中性
	...
	NotBeforeTime                emby.Time        `json:"not_before_time,omitempty"`                 // 管理性释放后的独立最早领取时间
	ForceRun                     bool             `json:"force_run,omitempty"`                       // 用户手动插队；消费一次后自动清除
	StateRevision                uint64           `json:"state_revision,omitempty"`                  // 持久化状态的单调版本，用于跨优先级快照去重
	ClaimToken                   uint64           `json:"-"`                                         // 进程内领取代次，拒绝迟到 worker 覆盖新状态
}
